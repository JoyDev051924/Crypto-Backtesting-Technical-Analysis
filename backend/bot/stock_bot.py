...
from bot.momentum_tracker import MomentumTracker
from bot.market_regime import MarketRegimeDetector
from bot.advanced_options import AdvancedOptionsTrader
from bot.notifications import EmailNotifier
...
class StockTradingBot:
    def __init__(self):
        ...
        self.market_open_buffer = 0  # No buffer - start trading immediately
        ...
        self.swing_trading_enabled = True  # Hold 90%+ confidence trades overnight
        self.swing_confidence_threshold = 90  # Only swing trade very high confidence
        self.swing_stop_loss_pct = 0.06  # Wider stops for swing trades (6%)
        self.premarket_trading_enabled = True  # Trade 7:00-9:30 AM EST
        self.premarket_confidence_threshold = 75  # Higher threshold for pre-market
        ...
        # Initialize options trader (30% allocation for higher returns)
        self.options_trader = AdvancedOptionsTrader(self.base_url, self.headers, cash_allocation=0.30)
        ...
        self.sentiment_analyzer = SentimentAnalyzer(self.headers)
        self.earnings_calendar = EarningsCalendar(self.headers)
        self.correlation_analyzer = CorrelationAnalyzer()
        self.adaptive_sizing = AdaptiveSizing()
        self.momentum_tracker = MomentumTracker(self.base_url, self.headers)
        self.market_regime_detector = MarketRegimeDetector(self.base_url, self.headers)
        
        # Initialize email notifications
        notification_email = os.getenv('NOTIFICATION_TO')
        self.notifier = EmailNotifier(notification_email) if notification_email and notification_email != 'your_email@gmail.com' else None
        ...
        print("  ✅ Intraday Momentum Tracking")
    ...
    def get_liquid_stocks(self):
        ...
        priority_stocks = [
            ...
            # Leveraged ETFs (3x leverage for extra returns)
            'TQQQ', 'SQQQ',  # 3x Nasdaq (bull/bear)
            'UPRO', 'SPXU',  # 3x S&P 500 (bull/bear)
            'SOXL', 'SOXS',  # 3x Semiconductors (bull/bear)
            'TNA', 'TZA',    # 3x Russell 2000 (bull/bear)
            'TECL', 'TECS',  # 3x Technology (bull/bear)
            'FAS', 'FAZ',    # 3x Financials (bull/bear)
            # Mega cap tech
    ...
    def calculate_confidence(self, indicators, regime):
        """Calculate confidence score for entry with AI/ML enhancements"""
        if regime != 'uptrend':
            return 0
    ...
    def calculate_enhanced_confidence(self, symbol, indicators, regime):
        """Enhanced confidence calculation with all AI/ML modules"""
        # Get base confidence
        base_confidence = self.calculate_confidence(indicators, regime)
        
        if base_confidence == 0:
            return 0
        
        # Apply ML pattern recognition
        confidence = self.ml_predictor.adjust_confidence(base_confidence, indicators, regime)
        
        # Apply sentiment analysis
        confidence = self.sentiment_analyzer.adjust_confidence_for_sentiment(confidence, symbol)
        
        # Apply earnings calendar check
        confidence = self.earnings_calendar.adjust_confidence_for_earnings(confidence, symbol, indicators)
        
        # Apply momentum tracking
        confidence = self.momentum_tracker.adjust_confidence_for_momentum(confidence, symbol)
        
        return max(0, min(100, confidence))

    
    def place_order(self, symbol, qty, side='buy', order_type='limit', limit_price=None):
    ...
    def close_position(self, symbol, exit_price, reason):
        """Close a position with better error handling and ML learning"""
        ...
        try:
            ...
            # Learn from this trade (ML)
            if 'indicators' in position and 'regime' in position:
                self.ml_predictor.record_trade_result(
                    position['indicators'],
                    position['regime'],
                    pnl_pct
                )
            ...
            self.adaptive_sizing.record_trade(pnl_pct, trade_record['exit_time'])
    ...
    def scan_for_entries(self):
        """Scan symbols for entry opportunities with AI/ML enhancements"""
        # Check if paused
        if self.is_paused:
        ...
        max_positions = regime_adjustments['max_positions']
        confidence_threshold = regime_adjustments['confidence_threshold']
        regime_multiplier = regime_adjustments['position_size_multiplier']
        ...
        # Get adaptive sizing multiplier
        adaptive_multiplier = self.adaptive_sizing.get_size_multiplier()
        perf_summary = self.adaptive_sizing.get_performance_summary()
        if perf_summary['status'] != 'insufficient_data':
            print(f"📊 Performance: {perf_summary['status']} | Size multiplier: {adaptive_multiplier:.2f}x")
        
        # Combine all multipliers
        total_multiplier = volatility_multiplier * regime_multiplier * adaptive_multiplier
        
        # Get top momentum stocks to prioritize
        top_momentum = self.momentum_tracker.get_top_momentum_stocks(self.symbols, limit=50)
        priority_symbols = [s['symbol'] for s in top_momentum if s['momentum_score'] >= 60]
        
        # Scan priority symbols first, then others
        scan_order = priority_symbols + [s for s in self.symbols if s not in priority_symbols]
        ...
        for symbol in scan_order:
            ...
            if not self.correlation_analyzer.check_correlation_limit(symbol, list(self.positions.keys())):
                continue
            ...
            try:
                ...
                confidence = self.calculate_enhanced_confidence(symbol, indicators, regime)
                ...
                if confidence >= confidence_threshold:
                    ...
                    if (self.options_enabled and 
                        ...
                        self.enter_option_position(symbol, indicators['price'], confidence, indicators, regime)
                    else:
                        # Trade stocks
                        self.enter_position(symbol, indicators['price'], confidence, total_multiplier, indicators, regime)
                    
                    if len(self.positions) + len(self.options_positions) >= max_positions:
                        break
            except Exception as e:
                print(f"Error scanning {symbol}: {e}")
                continue
    
    def enter_position(self, symbol, price, confidence, volatility_multiplier=1.0, indicators=None, regime=None):
        """Enter a new position with dynamic sizing and limit orders for better entry"""
        ...
        try:
            # Use limit order for better entry (0.3% below current price)
            limit_price = price * 0.997
            
            # Place buy order
            order = self.place_order(symbol, quantity, side='buy', order_type='limit', limit_price=limit_price)
            
            # Verify order was placed
            if not order or order.get('status') == 'rejected':
                print(f"❌ Order rejected for {symbol}, trying market order")
                # Fallback to market order
                order = self.place_order(symbol, quantity, side='buy', order_type='market')
                if not order or order.get('status') == 'rejected':
                    print(f"❌ Market order also rejected for {symbol}")
                    return
            
            # Record position with indicators for ML learning
            self.positions[symbol] = {
                ...
                'indicators': indicators,  # Store for ML learning
                'regime': regime  # Store for ML learning
            }
    ...
    def run(self):
        """Main bot loop"""
        print("=" * 60)
        print("🤖 ENHANCED AI STOCK TRADING BOT STARTED")
        print("=" * 60)
        ...
        try:
            ...
            print(f"   Account Type: {'LIVE TRADING' if 'paper' not in self.base_url else 'PAPER TRADING'}")
            print(f"   Cash Available: ${actual_cash:,.2f}")
            print(f"   Total Equity: ${actual_equity:,.2f}")
            ...
            print(f"❌ ERROR: Could not fetch account balance from Alpaca: {e}")
            print(f"⚠️  Using cached balance: ${self.cash:,.2f}")
            print("⚠️  This may be incorrect! Check your API credentials.")
        ...
        print(f"Market Regime: {regime.upper()} (confidence: {confidence:.0f}%)")
        print(f"Max Positions: {regime_adjustments['max_positions']} (regime-adjusted)")
        ...
        print("  ✓ Earnings Calendar Awareness")
        ...
        while True:
            try:
                ...
                if self.close_all_eod and self.should_close_all_positions():
                    ...
                    # Close day trading positions
                    for symbol in list(self.positions.keys()):
                        position = self.positions[symbol]
                        # Keep swing trades (90%+ confidence) overnight
                        if self.swing_trading_enabled and position.get('confidence', 0) >= self.swing_confidence_threshold:
                            print(f"📊 HOLDING {symbol} overnight (swing trade, confidence: {position['confidence']:.0f}%)")
                            continue
                    ...
                    print("✅ Day trading positions closed, swing trades held")
                ...
                if self.notifier:
                    ...
                    if self.last_notification_time is None or (current_time - self.last_notification_time).seconds >= 3600:
                        # Update current prices in positions
                        for symbol in self.positions:
                            bars = self.get_bars(symbol, limit=1)
                            if bars:
                                self.positions[symbol]['current_price'] = float(bars[-1]['c'])
    ...
    def enter_option_position(self, symbol, stock_price, confidence, indicators=None, regime=None):
        ...
        try:
            ...
            if not option:
                ...
                self.enter_position(symbol, stock_price, confidence, 1.0, indicators, regime)
                return
